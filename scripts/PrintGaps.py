...
ap.add_argument("--qpos", default=False, action='store_true',
                help="Write query position of gaps")
...
# Write header
if args.header:
    out_file.write('#CHROM\tPOS\tEND\tSVTYPE\tSIZE\tSEQ\tTSD\tREAD_ID\tREAD_POS\tREAD_END\n')
        ...
        for i in range(len(packedOps)):
                ...
                if out_snv_file is not None:
                    ...
                    for mp in range(0,len(targetSeq)):
                        ...
                        if querySeq[mp].upper() != targetSeq[mp].upper() and \
                            ...
                            out_snv_file.write(
                                "{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(
                                    aln.tName, tPos+mp, tPos+mp+1, targetSeq[mp], querySeq[mp], aln.title, mp+qPos
                                )
                            )
            ...
            if op == I:
                if oplen >= args.minLength and (args.maxLength is None or oplen < args.maxLength):
                    ...
                    if doPrint:
                        out_file.write(
                            "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format(
                                chrName, tPos, tPos + oplen, "INS", oplen, gapSeq,
                                "notsd", aln.title, qPos, qPos + oplen
                            )
                        )
                        ...
                        if args.qpos:
                            out_file.write(
                                "\t{}\t{}\t{}\t{}".format(
                                    qPos, qPos + len(gapSeq), aln.strand, len(aln.seq)
                                )
                            )
            ...
            if op == D:
                if oplen >= args.minLength and (args.maxLength is None or oplen < args.maxLength):
                    ...
                    out_file.write(
                        "{}\t{}\t{}\t{}\t{}\t{}\tnotsd\t{}\t{}\t{}".format(
                            chrName, tPos, tPos + oplen, "DEL", oplen, delSeq, aln.title, qPos, qPos + 1
                        )
                    )
                    ...
                    if args.qpos:
                        out_file.write("\t{}\t{}\t{}\t{}".format(qPos, qPos + 1, aln.strand, len(aln.seq)))
